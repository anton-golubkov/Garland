...
import ipf.ipfblock.erosion

class TestErosionBlock(unittest.TestCase):
    def setUp(self):
        self.block = ipf.ipfblock.erosion.Erosion()
        ...
        self.block.properties["iterations"].set_value(5)
    ...
    def test_output_image(self):
        self.block.process()
        output_image = self.block.output_ports["output_image"].get_value()
        cv.SaveImage("files/test_erosion_out.png", output_image)
        output_image = cv.LoadImage("files/test_erosion_out.png")
        erosion_image = cv.LoadImage("files/test_erosion.png")
