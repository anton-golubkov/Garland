...
import ipf.ipfblock.dilate

class TestDilateBlock(unittest.TestCase):
    def setUp(self):
        self.block = ipf.ipfblock.dilate.Dilate()
        ...
        self.block.properties["iterations"].set_value(2)
    ...
    def test_output_image(self):
        self.block.process()
        output_image = self.block.output_ports["output_image"].get_value()
        cv.SaveImage("files/test_dilate_out.png", output_image)
        output_image = cv.LoadImage("files/test_dilate_out.png")
        erosion_image = cv.LoadImage("files/test_dilate.png")
