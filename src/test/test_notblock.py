...
import ipf.ipfblock.not_block

class TestNotBlock(unittest.TestCase):
    def setUp(self):
        self.block = ipf.ipfblock.not_block.Not()
    ...
    def test_output_image(self):
        self.block.process()
        output_image = self.block.output_ports["output_image"].get_value()
        cv.SaveImage("files/test_not_out.png", output_image)
        output_image = cv.LoadImage("files/test_not_out.png")
        erosion_image = cv.LoadImage("files/test_not.png")
