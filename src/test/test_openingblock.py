...
import ipf.ipfblock.opening

class TestOpeningBlock(unittest.TestCase):
    def setUp(self):
        self.block = ipf.ipfblock.opening.Opening()
    ...
    def test_output_image(self):
        self.block.process()
        output_image = self.block.output_ports["output_image"].get_value()
        cv.SaveImage("files/test_opening_out.png", output_image)
        output_image = cv.LoadImage("files/test_opening_out.png")
        erosion_image = cv.LoadImage("files/test_opening.png")
